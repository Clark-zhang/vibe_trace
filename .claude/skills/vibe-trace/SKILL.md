---
name: vibe-trace
description: Capture, normalize, inspect, validate, and prepare local-first AI coding session traces with the Vibe Trace schema. Use when Claude needs to create or repair `.vibetrace.json` files, convert Claude Code/Codex/Cursor/manual histories into a unified trace, design or update Vibe Trace adapters, reason about checkpoints and Git state, run a local trace viewer/import workflow, or prepare a trace for privacy review and optional publishing.
---
...
1. Identify the task:
   - Create a new trace JSON: run `python3 "${CLAUDE_SKILL_DIR}/scripts/create_trace_skeleton.py"`.
   - Validate or repair an existing trace: run `python3 "${CLAUDE_SKILL_DIR}/scripts/check_trace.py" path/to/file.vibetrace.json` and load `references/trace.schema.json` as needed.
...
3. Validate before delivering:
   - `python3 "${CLAUDE_SKILL_DIR}/scripts/check_trace.py" path/to/file.vibetrace.json`
...
6. Validate the produced trace with `python3 "${CLAUDE_SKILL_DIR}/scripts/check_trace.py"`.
